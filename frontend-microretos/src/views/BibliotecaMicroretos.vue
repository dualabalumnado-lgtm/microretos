...
<template>
  <div class="min-h-screen bg-[#F8FAFC] p-4 md:p-12 transition-colors duration-500 font-sans text-[#1F2937] relative overflow-hidden">
    
    <div class="absolute top-[-10%] left-1/2 transform -translate-x-1/2 w-[800px] h-[500px] bg-[#99CC33] opacity-10 blur-[120px] rounded-full pointer-events-none transition-opacity duration-1000" :class="isLoaded ? 'opacity-10' : 'opacity-0'"></div>
    ...
    <div class="max-w-7xl mx-auto relative z-10">
      ...
      <header class="mb-10 text-center flex flex-col items-center">
        ...
        <div class="inline-flex items-center mb-8 bg-[#1F2937] py-4 pr-10 pl-6 rounded-[3rem] shadow-lg border border-[#333333] transition-all duration-1000 ease-out transform"
          ...
          <span class="font-black text-4xl md:text-5xl tracking-tighter uppercase text-white italic relative z-20">
            Dua<span class="text-[#00A859]">Lab</span><span class="text-[#99CC33] not-italic text-lg md:text-xl ml-1">Library</span>
          </span>
        </div>
        ...
        <h1 class="text-4xl md:text-5xl font-black tracking-tight mb-4 text-[#121212] transition-all duration-1000 delay-150 ease-out transform"
            :class="isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'">
          Explorador de <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#00A859] to-[#99CC33]">Micro-Retos</span>
        </h1>
      ...
      <section class="bg-white/90 backdrop-blur-md rounded-[2rem] p-6 md:p-8 border border-gray-100 shadow-[0_20px_50px_rgb(0,0,0,0.04)] mb-14 transition-all duration-1000 delay-500 ease-out transform"
               :class="isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
          
          <div>
            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-2 mb-2 flex items-center gap-2">
              <svg class="w-3 h-3 text-[#00A859]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
              Centro Educativo
            </label>
            <select v-model="filtroCentro" @change="filtroFamilia = ''; filtroCiclo = ''" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-[#1F2937] focus:bg-white focus:border-[#00A859] focus:ring-2 focus:ring-[#00A859]/10 outline-none transition-all shadow-sm">
          ...
          <div>
            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-2 mb-2 block">Familia Profesional</label>
            <select v-model="filtroFamilia" @change="filtroCiclo = ''" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-[#1F2937] focus:bg-white focus:border-[#00A859] focus:ring-2 focus:ring-[#00A859]/10 outline-none transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed" :disabled="familiasDisponibles.length === 0">
          ...
          <div>
            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-2 mb-2 block">Ciclo Formativo</label>
            <select v-model="filtroCiclo" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-[#1F2937] focus:bg-white focus:border-[#00A859] focus:ring-2 focus:ring-[#00A859]/10 outline-none transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed" :disabled="ciclosDisponibles.length === 0">
      ...
      <div class="transition-all duration-1000 delay-700 ease-out transform"
        ...
        <div v-if="cargando" class="flex flex-col items-center justify-center py-20">
          <svg class="animate-spin w-12 h-12 text-[#00A859] mb-4" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2v4a6 6 0 106 6h4a10 10 0 11-10-10z"/></svg>
        ...
        <div v-else-if="microretosFiltrados.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="reto in microretosFiltrados" :key="reto.id" 
            ...
            <div class="p-7 flex-1 flex flex-col">
              ...
              <div class="flex flex-wrap justify-end gap-2 mb-4 w-full">
                <div class="bg-[#00A859]/10 border border-[#00A859]/20 text-[#00A859] px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full backdrop-blur-sm">
                  {{ reto.nivel_grupo || 'Nivel ND' }}
                </div>

                <div v-if="reto.es_simulado" class="bg-[#1F2937] border border-gray-600 text-white px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full backdrop-blur-sm shadow-sm flex items-center gap-1">
                  <svg class="w-3 h-3 text-[#00A859]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                  Info Simulada
                </div>
              </div>
        ...
        <div v-else class="text-center py-20 bg-white rounded-[2rem] border border-dashed border-gray-300 shadow-sm">
          ...
          <button @click="limpiarFiltros" class="mt-6 px-6 py-2 bg-white border border-gray-200 hover:border-[#00A859] hover:text-[#00A859] text-[#1F2937] rounded-full text-xs font-bold uppercase tracking-widest transition-colors shadow-sm">
